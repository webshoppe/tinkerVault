...
| **Hermes Console** (v1.2.1) | Resolve Hermes approval-gated tool calls and browse sessions from any device, a phone-friendly PWA that talks straight to a running Hermes API server (approve/deny tool calls, chat, session browse + fork, per-message copy/rerun, session auto-naming). Latest: fixes approvals always failing (wrong request field), and replaces misleading "(unknown)"/"null" with an honest message when tool details aren't available over this transport | [Open](https://webshoppe.github.io/tinkerVault/apps/hermes-console/) | [`apps/hermes-console/`](apps/hermes-console/) |
| **Dossier** (v2.0.1) | Portable, offline dossier workspace for Windows: documents (with .odt/.ods import), notes, sticky notes linked to Kanban cards, paint, kanban, annotate, a decision timeline, and an Agenda view, plus full-text search, collections, and an optional local agent panel. Native compiled `.exe`, not a browser app. Gap-fix release: Notes/Decisions two-pane layout no longer squishes at a maximized window | [Download](https://github.com/webshoppe/tinkerVault/releases/tag/dossier-v2.0.1) | [`apps/dossier/`](apps/dossier/) |

Each **Try it** link above always points to that app's current version, hosted live via GitHub Pages. GitHub Pages only ever serves the current version, it doesn't host old releases live, so looking at an older version means downloading its `releases/vX.X.X/` folder below and opening its HTML file directly; it runs exactly the same way, just without the hosted link. Dossier is the one exception: it's a compiled Windows `.exe`, not a browser app, so its "Try it" column links to its GitHub Release download instead of a hosted page. The landing page itself lives at [webshoppe.github.io/tinkerVault](https://webshoppe.github.io/tinkerVault/).
...
- Hermes Console: [v1.2.1](apps/hermes-console/) current, [v1.2.0](apps/hermes-console/releases/v1.2.0/index.html), [v1.1.0](apps/hermes-console/releases/v1.1.0/index.html), and [v1.0.0](apps/hermes-console/releases/v1.0.0/index.html) archived
- Dossier: [v2.0.1](apps/dossier/) current, [v2.0.0](apps/dossier/releases/2.0.0/) and [v1.0.3](apps/dossier/releases/1.0.3/) archived
...
tinkerVault/
├── apps/
│   ├── hermes-console/  (currently v1.2.1)
│   │   └── releases/
│   │       ├── v1.0.0/  (archived)
│   │       ├── v1.1.0/  (archived)
│   │       ├── v1.2.0/  (archived)
│   │       └── v1.2.1/  (duplicate of current, kept as a snapshot)
...
│   └── dossier/  (currently v2.0.1, compiled Windows .exe, not a browser app)
│       └── releases/
│           ├── 1.0.3/  (archived)
│           ├── 2.0.0/  (archived)
│           └── 2.0.1/  (duplicate of current, kept as a snapshot)
├── docs/
│   └── PHILOSOPHY.md
├── DESKTOP-LAUNCHER.md
├── LICENSE
